...
export class CartComponent implements OnInit {
  ...
  totalPrice:any = 0
  faTrash=faTrash
  constructor(private router : Router ,private sourav : SouravService , private loader: NgxUiLoaderService) {
   ...
   removeCakeFromCart(index:any){
      ...
      this.loader.start()
      this.sourav.removeCakeFromCart(url,body,options).subscribe({
        next:(response:any)=>{
          this.loader.stop()
   ...
   increaseQuantity(index:any){
     ...
     this.loader.start()
     this.sourav.addCakeToCart(url,this.cartitems[index],options).subscribe({
       next:(response:any)=>{
         this.loader.stop()
         if(response.data){
   ...
   checkout(){
     ...
     this.sourav.price = this.totalPrice
